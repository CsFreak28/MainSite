...
function processPreWrittenMsg(msg) {
  if (msg.includes("Hey")) {
    let text = wrapWordWithSpan(
      "Hello there Gideon,\nRinovate is a company dedicated to building faster and more convenient tech for students like yourself \n because you're a customer too. . . therefore your satisfaction is compulsory.\nfeel free to ask me anything about Rinovate.",
      "Rinovate"
    );
    console.log(text);
...
function moveRinovateLogoUp(element) {
  console.log(element);
  let upSvg = element.querySelector(".upSvg");
  upSvg.classList.replace("upSvg", "downSvg");
  element.querySelector(".rinovateLogoSpace").style.width = "0px";
...
function appendRinoMessage() {
  ...
  rinovateLogoSpace.innerHTML += `<div class="innerLogoSpace"><svg
...
userInput.addEventListener("input", (e) => {
  ...
  let rionvateIsStillTyping = getRinovateIsTypingValue();
  if (value !== "") {
    // console.log(userTypingBubble);
    if (!rionvateIsStillTyping) {
